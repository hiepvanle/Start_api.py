...
class Album(Resource):
            ...
            with self.connections.cursor() as cursor:
                ...
                if request.args['aid'] == "*":
                    ...
                    for i in result:
                        data = {
                            'album_id': i[0],
                            'album_name': i[1],
                            'date_release_album': i[2].strftime('%Y-%m-%d %H:%M:%S'),
                ...
                # get by id
                    ...
                    data = {
                        'album_id': result[0],
                        'album_name': result[1],
                        'date_release_album': result[2].strftime('%Y-%m-%d %H:%M:%S'),
                        'album_description': result[3],
                    }
                    return data, 200
        else:
    ...
    def post(self):
        if request.is_json:
            ...
            with self.connections.cursor() as cursor:
                sql_post = "INSERT INTO `tbl_album` (`album_id`, `album_name`, " \
                           "`date_release_album`, `album_description`) " \
                           "VALUES ('{}', '{}','{}','{}');"
                sql_post = sql_post.format(data['album_id'], data['album_name'],
                                           data['date_release_album'], data['album_description'])
