...
class Song(Resource):
    ...
    def post(self):
        if request.is_json:
            ...
            with self.connections.cursor() as cursor:
                sql_post = "INSERT INTO `tbl_song` (`song_id`, `song_name`, `listen_count`,`rate`) " \
                           "VALUES ('{}', '{}','{}','{}');"
                sql_post = sql_post.format(data['song_id'], data['song_name'], data['listen_count'], data['rate'])
    ...
    def delete(self):
        if request.is_json:
            ...
            return {"error": "invalid request body"}, 404

    def put(self):
        if request.is_json:
            # convert to json
            data = request.get_json(force=True)
            sql_put = "UPDATE tbl_song SET `song_name`=%(song_name)s," \
                      " `listen_count`=%(listen_count)s WHERE `song_id`=%(song_id)s;"
            with self.connections.cursor() as cursor:
